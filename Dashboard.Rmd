...
if("Genre" %in% names(movies_clean) && !"Genre1" %in% names(movies_clean)) {
  movies_clean <- movies_clean %>%
    mutate(
      Genre1 = sapply(strsplit(as.character(Genre), ", "), function(x) x[1]),
      Genre2 = sapply(strsplit(as.character(Genre), ", "), function(x) ifelse(length(x) >= 2, x[2], NA)),
      Genre3 = sapply(strsplit(as.character(Genre), ", "), function(x) ifelse(length(x) >= 3, x[3], NA))
    )
}
```
...
years <- sort(unique(movies_clean$Released_year))
...
output$top20 <- renderPlotly({
  ...
  p <- ggplot(
    ...
    aes(x = BoxOffice_adjusted, y = reorder(Title, BoxOffice_adjusted), text = paste(
        "Box Office Revenue:", "$", scales::dollar(BoxOffice_adjusted),
        "<br>Title:", Title,
...
output$categ <- renderPlotly({
  ...
  # 检查列是否存在，如果不存在则创建
  if(input$group %in% c("Genre1", "Genre2", "Genre3") && !input$group %in% names(movies_clean)) {
    if(input$group == "Genre1") {
      movies_clean <- movies_clean %>%
        mutate(Genre1 = sapply(strsplit(as.character(Genre), ", "), function(x) x[1]))
    } else if(input$group == "Genre2") {
      movies_clean <- movies_clean %>%
        mutate(Genre2 = sapply(strsplit(as.character(Genre), ", "), 
                              function(x) ifelse(length(x) >= 2, x[2], NA)))
    } else if(input$group == "Genre3") {
      movies_clean <- movies_clean %>%
        mutate(Genre3 = sapply(strsplit(as.character(Genre), ", "), 
  ...
  categories <- movies_clean %>%
    ...
    group_by(Released_year, .data[[input$group]]) %>%
...
# Data
time <- movies_clean %>%
  group_by(Released_year) %>%  # 改为 Released_year
  summarize(
    count = n(),
...
output$plot <- renderPlotly({
  ...
  p <- ggplot(
    time,
    aes(x = Released_year, y = .data[[input$y]])
  ) +
    geom_point(color = "#480F8E", fill = "#480F8E") +
    geom_line(color = "#246A73") + 
    theme_minimal()+
