...
scenario_ml_results <- eventReactive(input$run_scenario_btn, {
  ...
  withProgress(message = 'Running ML models...', value = 0, {
    ...
    modified_features$Released_year <- input$scenario_year_slider
    modified_features$years_from_2000 <- input$scenario_year_slider - 2000
    ...
    pred_orig <- mean(c(pred_gam_orig, pred_rf_orig, pred_lm_orig))
    ...
    pred_mod <- mean(c(pred_gam_mod, pred_rf_mod, pred_lm_mod))
    ...
    list(
      ...
      predicted_original = pred_orig / 1e6,
      predicted_modified = pred_mod / 1e6,
      change_percent = (pred_mod - pred_orig) / pred_orig * 100
    )
  })
})
