...
IF NOT:
Creating a directory for local data 
```{r}
...
[RUN THIS] Obtain list of American movies from Wikipedia from the years 2000 - 2025 
```{r}
...
[RUN THIS] Clean wiki_films dataframe
```{r}
...
[RUN THIS] Extracting list of films to get data from the IMDB API 
```{r}
...
[RUN THIS] Split list of films into separate lists 
```{r}
...
Reading in saved data from IMDB API 
```{r}
data_1 <- readRDS("data/data_1.RDS")
data_2 <- readRDS("data/data_2.RDS")
data_3 <- readRDS("data/data_3.RDS")
data_4 <- readRDS("data/data_4.RDS")
data_5 <- readRDS("data/data_5.RDS")
data_6 <- readRDS("data/data_6.RDS")
data_7 <- readRDS("data/data_7.RDS")
data_8 <- readRDS("data/data_8.RDS")
```

Combining into large dataset 
```{r}
raw_data <- bind_rows(
  data_1,
  data_2,
...
#collapsing movies in mixed genre?
#how to handle directors, writers, cast?
#remove unncessary columns (e.g., poster, DVD, production, website, response)
```
