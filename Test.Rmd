...
create_seasonal_analysis<- function(data) {
  ...
  seasonal_data <- data %>%
    ...
    mutate(
      month = month(Released, label = TRUE, abbr = TRUE),
      month_num = month(Released),   # numeric for polar angle
      week = week(Released),
    ...
    summarise(
      ...
      blockbuster_pct =
        sum(BoxOffice_adjusted > 500000000, na.rm = TRUE) / n() * 100,
      .groups = "drop"
    ) %>%
    arrange(month_num)
  ...
  plot_ly(
    ...
    layout(
      title = list(
        text = "Seasonal Movie Release Patterns (Inflation-Adjusted)",
        x = 0.5
      ),
      polar = list(
        radialaxis = list(
          title = "Average Revenue (Million USD)",
          showline = FALSE,
